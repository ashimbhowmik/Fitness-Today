"use client";

import { toast } from "react-toastify";
import Notification from "../Notification/Notification";
import { addToCart } from "@/services/cart";
import { useContext } from "react";
import { GlobalContext } from "@/context/GlobalContext";
import ComponentLevelLoader from "../Loader/ComponentLevelLoader/ComponentLevelLoader";
import Footer from "../Footer/Footer";

export default function CommonDetails({ item }) {
  const {
    setComponentLevelLoader,
    componentLevelLoader,
    user,
    setShowCartModal,
  } = useContext(GlobalContext);

  async function handleAddToCart(getItem) {
    setComponentLevelLoader({ loading: true, id: "" });

    const res = await addToCart({ productID: getItem._id, userID: user._id });

    if (res.success) {
      toast.success(res.message, {
        position: toast.POSITION.TOP_CENTER,
      });
      setComponentLevelLoader({ loading: false, id: "" });
      setShowCartModal(true);
    } else {
      toast.error(res.message, {
        position: toast.POSITION.TOP_CENTER,
      });
      setComponentLevelLoader({ loading: false, id: "" });
      setShowCartModal(true);
    }
  }

  return (
    <main>
      <section className="mx-auto px-4 sm:px-6 lg:px-8 ">
        <div className="container mx-auto px-4 pt-[85px]">
          <div className="lg:col-gap-12 xl:col-gap-16 mt-8 grid grid-cols-1 gap-12 lg:mt-12 lg:grid-cols-5 lg:gap-16">
            <div className="lg:col-span-3 lg:row-end-1">
              <div className="lg:flex lg:items-start mt-10 lg:mt-0">
                <div className="lg:order-2 lg:ml-5">
                  <div className=" overflow-hidden rounded-lg">
                    <img
                      src={item?.imageUrl}
                      className="lg:h-[700px] h-[300px] object-cover"
                      alt="Product Details"
                    />
                  </div>
                </div>
                <div className="mt-6 w-full lg:order-1 lg:w-32 lg:flex-shrink-0">
                  <div className="flex flex-row items-start lg:flex-col">
                    <button
                      type="button"
                      className="flex-0 aspect-square mb-3 h-20 overflow-hidden rounded-lg border-2 border-gray-100 text-center"
                    >
                      <img
                        src={item?.imageUrl}
                        className="h-full  object-cover"
                        alt="Product Details"
                      />
                    </button>
                    <button
                      type="button"
                      className="flex-0 aspect-square mb-3 h-20 overflow-hidden rounded-lg border-2 border-gray-100 text-center"
                    >
                      <img
                        src={item?.imageUrl}
                        className="h-full  object-cover"
                        alt="Product Details"
                      />
                    </button>
                  </div>
                </div>
              </div>
            </div>
            <div className="lg:col-span-2 lg:row-span-2 lg:row-end-2 mt-3">
              <h1 className="text-3xl font-bold">{item && item?.name}</h1>
              <div className="mt-8 flex flex-col items-center justify-between space-y-4 botder-t border-b py-4 sm:flex-row sm:space-y-0">
                <div className="flex items-end">
                  <h1
                    className={`text-3xl font-bold mr-2 ${
                      item?.onSale === "yes" ? "line-through" : ""
                    }`}
                  >
                    ${item && item?.price}
                  </h1>
                  {item?.onSale === "yes" ? (
                    <h1 className="text-3xl font-bold text-red-700">{`$${(
                      item?.price -
                      item?.price * (item?.priceDrop / 100)
                    ).toFixed(2)}`}</h1>
                  ) : null}
                </div>
                <button
                  type="button"
                  onClick={() => handleAddToCart(item)}
                  className="mt-1.5 inline-block bg-black px-5 py-3 text-xs font-medium tracking-wide uppercase text-white"
                >
                  {componentLevelLoader && componentLevelLoader.loading ? (
                    <ComponentLevelLoader
                      text={"Adding to Cart"}
                      color={"#ffffff"}
                      loading={
                        componentLevelLoader && componentLevelLoader.loading
                      }
                    />
                  ) : (
                    "Add to Cart"
                  )}
                </button>
              </div>

              <div className="lg:col-span-3 mt-3">
                <div className="border-b border-gray-400">
                  <nav className="flex gap-4">
                    <a
                      href="#"
                      className="border-b-2 border-[#111] py-4 text-xl font-medium "
                    >
                      Description
                    </a>
                  </nav>
                </div>
                <div className="mt-8 flow-root sm:mt-12">
                  {item && item?.description}
                </div>
              </div>
            </div>
          </div>
        </div>
        <Notification />
      </section>
      <Footer></Footer>
    </main>
  );
}
